...
# NOTE: there will be warnings, but it's because Morton Arb doesn't have 2 parts tot he name
...
MalusTotal_Reduction <- cor(Malus_climate_total[,which(sapply(Malus_climate_total, class)=="numeric")])
...
QuercusTotal_Reduction <- cor(Quercus_climate_total[,which(sapply(Quercus_climate_total, class)=="numeric")])
...
TiliaTotal_Reduction <- cor(Tilia_climate_total[,which(sapply(Tilia_climate_total, class)=="numeric")])
...
UlmusTotal_Reduction <- cor(Ulmus_climate_total[,which(sapply(Ulmus_climate_total, class)=="numeric")])
